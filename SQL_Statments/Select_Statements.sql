...
SELECT  C.CName, C.CNumber, Se.LectureSection, Le.Semester
FROM    Course AS C, Student_Takes AS ST, Student AS S, Section AS Se
WHERE   C.CName = ST.CName AND S.Email = ST.StuEmail AND Se.CName = C.CName, Se.CNumber = C.CNumber;
...
SELECT  C.MeetingName, C.Room#, C.Topic, C.CName, C.CNumber, L.Learning_Obj, L.Ch_dicussed, STS.Dayofweek, STS.TimeOfDay, STS.Frequency
FROM    Class_Meeting AS C, Lecture AS L, Course AS Co, Attends_Class AS AC, Student AS S, Scheduled_Time_Slot AS STS
WHERE   C.CName = Co.CName AND C.CNumber = Co.CNumber AND L.MeetingName = C.MeetingName AND AC.CMeetingName = C.MeetingName 
        AND AC.SEmail = S.Email AND STS.MeetingName = C.MeetingName;

SELECT  C.MeetingName, C.Room#, C.Topic, C.CName, C.CNumber, L.LabTopic, L.Due_Date, L.TA_Name, STS.Dayofweek, STS.TimeOfDay, STS.Frequency
...
SELECT  C.MeetingName, C.Room#, C.Topic, C.CName, C.CNumber, T.TA_Name, STS.Dayofweek, STS.TimeOfDay, STS.Frequency
FROM    Class_Meeting AS C, Tutorial AS T, Course AS Co, Attends_Class AS AC, Student AS S, Scheduled_Time_Slot AS STS
WHERE   C.CName = Co.CName AND C.CNumber = Co.CNumber AND T.MeetingName = C.MeetingName AND AC.CMeetingName = C.MeetingName 
        AND AC.SEmail = S.Email AND STS.MeetingName = C.MeetingName;

SELECT  C.MeetingName, C.Room#, C.Topic, C.CName, C.CNumber, Sp.Name, Sp.Organization, Sp.Credentials, STS.Dayofweek, STS.TimeOfDay, STS.Frequency
...
WHERE   C.CName = Co.CName AND C.CNumber = Co.CNumber AND Se.MeetingName = C.MeetingName AND AC.CMeetingName = C.MeetingName 
...
WHERE   G.CName = C.CName AND G.CNumber = C.CNumber AND GD.GroupID = G.ID AND GMM.GroupID = G.ID AND AGM.MeetingID = G.ID
